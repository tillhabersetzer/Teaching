% function make_chirps_flat (fstart, fstop, period, a)
function make_chirps_flat (fstart, fstop, fsamp, period, a, duration, scalefactor)
...
plot(cb);
fft_cb = fft(cb);
cb = real(ifft(cos(angle(fft_cb)) + 1i * sin(angle(fft_cb))));
...
%
first = 0;	% to override following if

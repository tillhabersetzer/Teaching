function experiment_click_focusrite(f,Settings,name)
%--------------------------------------------------------------------------
% parameters: 
%
% f:        figure handle
...
%--------------------------------------------------------------------------
% Updates for focusrite: 
% - change Driver
% - only 2 line outputs: use left for audio, right for trigger
%--------------------------------------------------------------------------
ASIODriver = 'Focusrite USB ASIO';
Stim       = Stim(:,1); % only left channel
...
ok = soundmexpro('trackmap','track', [0 1]); % virtual channel (first track -> channel 0, second track -> channel 1, third track -> channel 3)        
...
% optional: Waiting time before stimulus presentation
% pause(2);
...
for n = 1:Nstim
    ...
    ok = soundmexpro('loadmem', ...            % command name
        'data', [Stim, stim_trigger], ...      % matrix with one or more columns of data
        'track',[0 1], ...                   % vector with tracks, where data to be played
...
disp("Wiedergabe von experiment_click_focusrite.m erfolgreich beendet")

function experiment_click_loadandwait_focusrite()
%--------------------------------------------------------------------------
...
for n = 1:10
    ok = soundmexpro('loadmem', ...
     'data',vertcat([fsign*Stim, stim_trigger],zeros(JitterDuration(tracks_loaded+1),2)), ...
     'track',[0 1], ...
     'loopcount', 1);
    if ~ok,  error('error calling ''loadmem''' ); end
...
ok = soundmexpro('start','length', 0);  % 0 or L           
...
pause(5) % length of pause should be as long as buffer 10*stimulus
...
%---------------------
[x,fs] = audioread('out_1.wav');

figure
plot(x);

onsets = find(x(1:end-1)-x(2:end)>0);

% check jitter
jitter = diff(onsets)*1000/44100; % in ms
figure
histogram(jitter)
max(jitter)
min(jitter)

...
% 17.08.2028 tidy up
%
% 13.08.2018 TrigInfo.NumOfBits added to return number of Triggerbits for
% the respective EEG-system
%
...
% comment 17.8. should be almost useless meanwhile
%
%


function [TrigInfo] = EEGTrigID2info(TrigID,BitSplitScheme,System,TrigDur,ASIO,BitRef,NumOfBits)
%
%[TrigInfo] =  EEGTrigID2info(TrigID,BitSplitScheme,System[,TrigDur,ASIO,BitRef,NumOfBits])
%
...
%                  [NumOfBits 0]
% System         - 'BioSemi','SynAmps' is needed to consider differences in the
...
% For the system "MEG" the parameter 'BitSplitScheme' is used in addition
% todetermine how many bits are used for triggersignal from the stimulus PC
% and how many bits are used for any response devices from the MEG system.
% The sum of the values in 'BitSplitScheme' (between 12 and 16) provide the
% numbe rof PC trigger-channels, the remaining channels are free for input
% device triggers (set the respective hardware switches!).
%
%
...
if isempty(ASIO),           ASIO.Driver   = 'ASIO Fireface USB';
    ASIO.SampFreq = 48000;
    ASIO.TrigChan = 8; %ASIO.TrigChan = 8;
end
...
switch System
    case {'MEG','meg','triux'} %,'standard','default'}
        ...
        NumOfTrigCh = sum(BitSplitScheme);
        NumOfBits = NumOfTrigCh
        if NumOfTrigCh < 12
            BitSplitScheme(end) = BitSplitScheme(end) + (12-NumOfTrigCh);
            NumOfTrigCh = 12;
            NumOfBits   = 12
            error('Number of Trigger Channels are set to the minimum allowed value 12, last BitSplitScheme value is extended respectively');
        elseif NumOfTrigCh > 16
            NumOfTrigCh = 16;
            warning('Number of Trigger Channels are set to maximum allowed value 16');            
        end

        if ismember(NumOfTrigCh,[13:16])
            message = sprintf('Please set the "Response" switches 13-%d to "Stim PC" and all others to "MEG"',NumOfTrigCh);
            warning(message);
            %f = msgbox(message);
            %uiwait(f);
        end        
...
switch System
    %---------------------------------------------
    case {'MEG','meg','triux','standard','default'} % still work in progress
        Bitmask = logical(TrigID_-'0');
        BM = zeros(size(Bitmask,1),16);
        BM(1:size(Bitmask,1),1:size(Bitmask,2)) = Bitmask;
        TrigInfo.TrigBitmask  = BM;
        [x,y]                 = find(BM);
        for k = 1:max(x)
            TrigInfo.TrigLED{k,1} = uint8(y(x==k))';
        end
        TrigInfo.TrigInt      = uint16(bin2dec(num2str(BM)));
        TrigInfo.TrigIntNom   = uint16(bin2dec(num2str(Bitmask)));
        TrigInfo.System       = System;
        TrigInfo.NumOfBits    = NumOfBits;
        
        %================================================================
        TrigInfo.TrigWord  = (sum((2.^repmat((0:1:NumOfBits-1),size(Bitmask,1),1)).*Bitmask,2))./(2.^BitRef);
        %================================================================
    ...
    case {'Synamps','synamps','Syn','syn','SynAmps','synAmps'} % for any reason (most probably cable) the bitconfiguration for the synamps ist flipped. To keep it consistent only the Triggerword is adapted respectively
        ...
        for t = 1:size(Bitmask,1)
            for g = 1:size(BitSplitScheme,2)
                TrigInfo.TrigLEDMulti{t,g}  = find( Bitmask(t, sum(BitSplitScheme(1:g-1))+1:sum(BitSplitScheme(1:g)) ) );
                Bitmask_ = fliplr(Bitmask);
                TrigInfo.TrigIntMulti(t,g)  = bin2dec( strrep( num2str(  fliplr(Bitmask_(t, sum(BitSplitScheme(1:g-1))+1:sum(BitSplitScheme(1:g)) ) ) ) ,' ','') );
            end
        end
...
% out of the original Triggerbox Demo using playrec by Stefan Toepken/Sarah
% Verhulst
%
% switch TRIG
%     case 1

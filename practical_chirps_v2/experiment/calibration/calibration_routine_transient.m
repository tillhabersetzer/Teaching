...
%% 4.) Check calibration
% - Fill calibration values in settings
% - Playback of calibrated stimulus
%   - Measure p-p Voltage
%   - Playback and adjust sine (2) to same voltage and measure level
% Alternatively
% - Calibrate in dB peSPL
...
%--------------------------------------------------------------------------
function check_calibration(channel, filename, target_level, cal_val, show_audio_tracks)
% Playback of calibration signal. The calibration signal (e.g. transient)
% is concatenated in an infinite loop. 
% Therefore, the transient is stretched with 0s so that it has 1 sec duration.
% Transient: Calibration with max-abs-value
...
%   Desired sound pressure level (SPL) at playback, in decibels.
%   default: 80 dB (p-p) peSPL):
% 
% cal_val: 1x2 array
%   Calibration values (in dB) for each channel. This array must include:
%   default: [110, 110]):
%   Left channel calibration value: cal_val(1)
%   Right channel calibration value: cal_val(2)
%
% show_audio_tracks: bool
% Shows visualization of files/vectors in tracks with SoundMexPro.
% True
...
fprintf('Calibration value used: %.1f dB.\n', cal_val)
fprintf('Initial Stimulus Level: %.1f dB FS (peak).\n',level_stim_init)
fprintf('Stimulus Level is changed by %.1f dB.\n', gain_dB)
fprintf('Adjusted stimulus Level: %.1f dB FS (peak).\n', level_stim)
fprintf('Expected Level in SLM: %.1f dB (p-p) peSPL.\n', target_level)
